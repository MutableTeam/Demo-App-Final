  ...
  useEffect(() => {
    ...
    if (platformType === "desktop") {
      ...
      const handleMobileInput = (inputState: GameInputState) => {
        ...
        // Movement - Use precise vectors when available, fallback to booleans
        if (inputState.movement.magnitude > 0) {
          // Use precise movement vectors for smooth analog movement
          player.controls.moveX = inputState.movement.vectorX * inputState.movement.magnitude
          player.controls.moveY = inputState.movement.vectorY * inputState.movement.magnitude
        } else {
          player.controls.moveX = 0
          player.controls.moveY = 0
        }

        // Keep boolean controls for compatibility
