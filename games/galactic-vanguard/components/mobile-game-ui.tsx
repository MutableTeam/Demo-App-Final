...
export default function MobileGameUI({ score, timedAbilities, onTimedAbilityClick }) {
  ...
  return (
    <>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/95 to-black/80 border-t border-cyan-500/30 p-4 pb-6 z-20">
        <div className="flex gap-8 justify-center pointer-events-auto mb-3">
          {timedAbilities.map((ability, index) => {
            ...
            return (
              <button
                ...
                className={`w-28 h-28 rounded-xl border-2 transition-all duration-200 relative overflow-hidden ${
                  ability.cooldown > 0
                ...
                <div className="flex flex-col items-center justify-center h-full relative z-10">
                  {index === 0 ? (
                    ...
                    <div className="w-12 h-12 mb-1 flex items-center justify-center">
                      <svg viewBox="0 0 24 24" className="w-10 h-10 fill-red-400">
                  ...
                  <span className="text-sm text-gray-300 leading-tight font-medium">{ability.name.split(" ")[0]}</span>
                </div>
        ...
        <div className="flex justify-center">
          <div className="text-xs text-orange-400 text-center font-semibold">TIMED ABILITIES</div>
        </div>
      </div>
    </>
  )
